...
TH1F* GetEfficiency( TH1F* hnum, TH1F* hden ) {
  TH1F* heff = (TH1F*)hnum->Clone( (std::string("fakerat_") + hnum->GetName()).c_str() );
  for( int ibin=1 ; ibin <=hnum->GetNbinsX(); ibin++ ){
    int npass  = hnum->GetBinContent( ibin );
    int ntotal = hden->GetBinContent( ibin ); 
    // Confidence interval
    double upper = TEfficiency::Wilson( ntotal, npass, 0.683, true);
    double lower = TEfficiency::Wilson( ntotal, npass, 0.683, false);
    heff->SetBinContent( ibin, (upper + lower) /2.0 );
    heff->SetBinError(   ibin, upper - (upper + lower) /2.0 );
  }
  heff->SetTitle("Wilson score efficiency");
  return heff;
}

void CompareEffFR( TH1F* heff, TH1F* hFR){
  for( int ibin=1; ibin< heff->GetNbinsX(); ibin++ ){
    std::cout << " ibin " << ibin << " FR " << hFR->GetBinContent(ibin) << " Eff " << heff->GetBinContent(ibin) << std::endl;
  }
}
...
void CalFR(){
  TFile *ifile = new TFile("/data/users/fengyb/FRCal4/result/root/result_Cut1_CSV_SF_Eff.root");
  
  TH1F *hpassVI  = (TH1F*)ifile->Get("Dfakerates/rebinned_pass_GJetData_TypeVI_nTrack");
  TH1F *htotalVI = (TH1F*)ifile->Get("Dfakerates/rebinned_total_GJetData_TypeVI_nTrack");
  TH1F *heffVI = GetEfficiency( hpassVI, htotalVI );

  TH1F *hpassV   = (TH1F*)ifile->Get("Dfakerates/rebinned_pass_GJetData_TypeV_nTrack");
  TH1F *htotalV  = (TH1F*)ifile->Get("Dfakerates/rebinned_total_GJetData_TypeV_nTrack");
  TH1F *heffV  = GetEfficiency( hpassV, htotalV );

  TH1F *hfracVI = (TH1F*)ifile->Get("fraction_GJetData_TypeVI");
  TH1F* hfracV  = (TH1F*)ifile->Get("fraction_GJetData_TypeV" );

  TH1F* hfb = (TH1F*)heffVI->Clone("fakerate_GJetData_BJet_calc");
  TH1F* hfl = (TH1F*)heffVI->Clone("fakerate_GJetData_LightJet_calc");

  FrHistoCal( hfracV, hfracVI, heffV, heffVI, hfb, hfl);

  TFile *ofile = new TFile("result.root", "RECREATE");
  hfb->SetDirectory( ofile );
  hfl->SetDirectory( ofile );
  ofile->Write();
}
